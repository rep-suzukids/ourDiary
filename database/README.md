...
19. Vercelの環境変数に `GOOGLE_CLIENT_ID` を追加します。値は `VITE_GOOGLE_CLIENT_ID` と同じです。
20. 再デプロイし、登録した管理者Googleアカウントでログインします。
...
リアクションは全ロールが閲覧・追加・解除でき、ピッカーの並び順に利用者ごとの直近利用履歴を使用します。
