...
import { getMonthlyTemperatureSummary } from '../services/temperatureApi.js'
import '../Milk.css'
...
function TemperatureCalendarPage({ session, onNavigate }) {
  ...
  useEffect(() => {
    ...
    setStatus('loading')
    setError('')
    getMonthlyTemperatureSummary(activeFamily.id, year, month)
      .then((result) => {
        if (!isActive) return
  ...
  return (
    <main className="milk-page milk-calendar-page temperature-page temperature-calendar-page">
      <header className="milk-page-header milk-calendar-header">
        ...
        <a
          className="milk-add-button temperature-add-button"
          href={`/temperature/new?date=${initialRecordDate}`}
          onClick={navigateLink(`/temperature/new?date=${initialRecordDate}`)}
        >
          ＋ 記録
        </a>
      </header>

      <section className="milk-calendar-card" aria-label={`${year}年${month}月の体温カレンダー`}>
          ...
          {calendarDays.map((day, index) => {
            ...
            return (
              <button
                ...
                aria-label={`${month}月${day}日の体温記録を見る`}
                onClick={() => onNavigate(`/temperature?date=${key}`)}
              >
                <span className="milk-calendar-day__number">{day}</span>
                <span className="milk-calendar-day__records temperature-calendar-day__records">
                  {children.map((child) => {
        ...
        <p className="milk-calendar-note">日付を選ぶと、その日の検温記録を確認できます。</p>
      </section>
    </main>
  )
}

export default TemperatureCalendarPage
